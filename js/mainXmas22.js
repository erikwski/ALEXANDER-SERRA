(function ($) {
  ...
  function checkPacchettiRimasti() {
    ...
    aux_dt = new Date("11/25/2022 06:00");
    if (oggi > aux_dt) num_coaching = num_coaching - 2;
    aux_dt = new Date("11/25/2022 10:00");
    if (oggi > aux_dt) num_coaching = num_coaching - 3;
    aux_dt = new Date("11/25/2022 13:00");
    if (oggi > aux_dt) num_coaching = num_coaching - 4;
    aux_dt = new Date("11/25/2022 16:00");
    if (oggi > aux_dt) num_coaching = num_coaching - 2;
    aux_dt = new Date("11/25/2022 19:00");
    if (oggi > aux_dt) num_coaching = num_coaching - 2;
  ...
  function gestioneModalePagamenti() {
    ...
    $("#show_dati_bonifico").click(() => {
      ...
      datiRichiestaBonifico({
        paypal_name: $("#name_buyer").val(),
        paypal_address: `${$("#address_buyer").val()}, ${$(
          "#city_buyer"
        ).val()} - ${$("#cap_buyer").val()} (${$("#prov_buyer").val()})`,
        paypal_mail: $("#email_buyer").val(),
        order_id: "",
        payer_id: "",
  ...
  function datiRichiestaBonifico(dati) {
    try {
      localStorage.setItem("user", JSON.stringify(dati));
      $.post("api/richiestaBonifico.php", dati).done((j) => {});
    } catch (error) {}
  }
